...
#### 1.2.1 Total Trip by Mode

The chart below illustrates the total number of trips recorded by each mode of transportation. With over 2 million observations, 88% of the trips were made by bus, with the remaining trips taken by trolley, trolley bus, and metro. It's important to note that these trip counts differ from ridership numbers, which are based on the number of passengers. Here, trips are counted by the number of stops each mode makes; hence, a single trip can account for multiple passengers depending on the vehicle's capacity and utilization.

This study focuses primarily on bus trips, which constitute the majority of the observations. For this analysis, trolley buses are also categorized under bus trips, which collectively account for over 90% of the trips. The subsequent sections will therefore focus on the bus trips.  
